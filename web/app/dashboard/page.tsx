...
const PLAN_META: Record<string, { label: string; color: string; bg: string }> = {
  trial_basic: { label: "Trial · Basic",  color: "#fbbf24", bg: "rgba(251,191,36,0.1)"   },
...
// ── Genesis Orb (empty state) ─────────────────────────────────────────────────

function GenesisOrb({ onClick }: { onClick: () => void }) {
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 28, padding: "48px 24px" }}>
      <div style={{ position: "relative", width: 160, height: 160, cursor: "pointer" }} onClick={onClick}>
        <div style={{
          position: "absolute", inset: -20, borderRadius: "50%",
          background: "conic-gradient(from 0deg,transparent 0%,rgba(124,58,237,0.45) 30%,rgba(56,189,248,0.4) 60%,transparent 100%)",
          filter: "blur(12px)", animation: "spin 5s linear infinite",
        }} />
        <div style={{
          position: "absolute", inset: 0, borderRadius: "50%",
          background: "radial-gradient(circle at 38% 35%,rgba(192,132,252,0.9),rgba(124,58,237,0.55) 50%,rgba(37,99,235,0.35) 75%,transparent)",
          boxShadow: "0 0 60px rgba(124,58,237,0.35),inset 0 0 30px rgba(56,189,248,0.12)",
        ...
        <div style={{
          ...
          <svg width="28" height="28" viewBox="0 0 28 28" fill="none">
            <path d="M14 4v20M4 14h20" stroke="white" strokeWidth="2.2" strokeLinecap="round" />
          </svg>
        </div>
      </div>
      <div style={{ textAlign: "center" }}>
        <h2 style={{ margin: "0 0 8px", fontFamily: "var(--font-display)", fontWeight: 700, fontSize: "1.35rem", color: "var(--color-starlight)", letterSpacing: "-0.02em" }}>
          Build your first agent
        </h2>
        <p style={{ margin: "0 0 24px", fontSize: "0.86rem", color: "var(--color-dust)", lineHeight: 1.6, maxWidth: 340 }}>
          Your AI agent handles customer questions, captures leads, and works 24/7 — set up in minutes.
        </p>
        <button className="btn-genesis" onClick={onClick}>
          Create agent
        </button>
      </div>
    </div>
...
function AgentCard({
  agent, onEdit, onDelete, onViewLeads,
}: {
  agent: Agent;
  onEdit: () => void;
  onDelete: () => void;
      ...
      <div style={{ padding: "20px 22px" }}>
        ...
        <div style={{ display: "flex", alignItems: "flex-start", gap: 14, marginBottom: 16 }}>
          ...
          <div style={{
            ...
            boxShadow: `0 0 22px rgba(${rgb},0.3)`,
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <h3 style={{ margin: "0 0 2px", fontFamily: "var(--font-display)", fontWeight: 700, fontSize: "1rem", color: "var(--color-starlight)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
              {agent.name}
            </h3>
            <p style={{ margin: 0, fontSize: "0.78rem", color: "var(--color-dust)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
              {agent.business_name}
            </p>
            {agent.tagline && (
              <p style={{ margin: "3px 0 0", fontSize: "0.73rem", color: "var(--color-dust)", opacity: 0.7, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                {agent.tagline}
              </p>
            )}
          </div>
          ...
          <div style={{ position: "relative", width: 40, height: 40, flexShrink: 0 }}>
            ...
            <div style={{
              ...
              color: health, fontFamily: "var(--font-mono)",
            }}>
              {pct}
            </div>
          </div>
        ...
        <div style={{
          ...
          <svg width="11" height="11" viewBox="0 0 12 12" fill="none">
            <path d="M2 10L10 2M10 2H5M10 2v5" stroke="var(--color-stellar)" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round" />
          </svg>
        </div>
        ...
        {agent.leads_pin && (
          <div style={{ marginBottom: 16, display: "flex", alignItems: "center", gap: 10 }}>
            ...
            {showPin
              ? <span style={{ fontFamily: "var(--font-mono)", fontWeight: 700, fontSize: "0.85rem", color: "var(--color-starlight)", letterSpacing: "0.1em" }}>{agent.leads_pin}</span>
              : <span style={{ fontFamily: "var(--font-mono)", fontSize: "0.85rem", color: "var(--color-dust)", letterSpacing: "0.1em" }}>• • • • • •</span>
            }
            <button onClick={() => setShowPin(!showPin)} style={{ background: "none", border: "none", cursor: "pointer", padding: "2px 6px", fontSize: "0.68rem", color: "var(--color-nebula)", fontFamily: "var(--font-mono)" }}>
              {showPin ? "hide" : "show"}
            </button>
          </div>
        )}
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <button onClick={onEdit} style={{
            ...
            color: color, fontSize: "0.82rem", fontWeight: 600, cursor: "pointer",
          ...
          <button onClick={onViewLeads} style={{
            flex: 2, padding: "9px 0", borderRadius: 11,
            background: "rgba(255,255,255,0.04)", border: "1px solid var(--line)",
            color: "var(--color-starlight)", fontSize: "0.82rem", cursor: "pointer",
          ...
          <button onClick={onDelete} title="Delete agent" style={{
          ...
          >
            <svg width="15" height="15" viewBox="0 0 15 15" fill="none">
              <path d="M2.5 4h10M5.5 4V2.5h4V4M6.5 7v4M8.5 7v4M3.5 4l.8 8.5h6.4L11.5 4" stroke="#f87171" strokeWidth="1.3" strokeLinecap="round" strokeLinejoin="round" />
            </svg>
          </button>
        </div>
      </div>
...
function TrialBanner({ plan, daysLeft }: { plan: string; daysLeft: number | null }) {
  ...
  return (
    <div style={{
      ...
      background: expired ? "rgba(248,113,113,0.08)" : "rgba(251,191,36,0.07)",
      border: `1px solid ${expired ? "rgba(248,113,113,0.25)" : "rgba(251,191,36,0.22)"}`,
      ...
      <p style={{ margin: 0, fontSize: "0.82rem", color: "var(--color-starlight)", flex: 1 }}>
        {expired
          ...
          : `${daysLeft} day${daysLeft === 1 ? "" : "s"} left in your ${plan === "trial_pro" ? "Pro" : "Basic"} trial.`
        }
      </p>
      <a href="/pricing" style={{
        padding: "7px 14px", borderRadius: 9,
        background: expired ? "rgba(248,113,113,0.15)" : "rgba(251,191,36,0.12)",
        border: `1px solid ${expired ? "rgba(248,113,113,0.3)" : "rgba(251,191,36,0.25)"}`,
        fontSize: "0.78rem", fontWeight: 600, textDecoration: "none",
        color: expired ? "#f87171" : "#fbbf24", whiteSpace: "nowrap",
      }}>
        Upgrade →
      </a>
    </div>
  ...
  return (
    <>
      <style>{`
        ...
        .btn-genesis { display:inline-flex;align-items:center;justify-content:center;gap:8px;padding:0.7rem 1.4rem;border-radius:12px;background:linear-gradient(135deg,#7c3aed,#2563eb,#0ea5e9);border:none;color:#fff;font-weight:600;font-size:0.9rem;font-family:var(--font-sans);cursor:pointer;box-shadow:0 0 28px rgba(124,58,237,0.35);transition:all 0.2s;text-decoration:none; }
        .btn-genesis:hover { filter:brightness(1.08);box-shadow:0 0 40px rgba(124,58,237,0.5);transform:translateY(-1px); }
        .btn-ghost { display:inline-flex;align-items:center;gap:6px;padding:0.65rem 1.3rem;border-radius:11px;border:1px solid var(--line);background:rgba(255,255,255,0.03);color:var(--color-starlight);font-size:0.88rem;cursor:pointer;text-decoration:none;font-family:var(--font-sans);transition:border-color 0.15s; }
      ...
      <div style={{ minHeight: "100vh", background: "var(--color-void)", backgroundImage: "radial-gradient(900px 500px at 70% -8%,rgba(124,58,237,0.09),transparent 65%)" }}>
        ...
        <header style={{
          ...
          <div style={{ maxWidth: 1160, margin: "0 auto", padding: "14px 24px", display: "flex", alignItems: "center", gap: 14 }}>
            {/* Back arrow if editing */}
            {editingId && (
              <button onClick={() => setEditingId(null)} style={{
                background: "none", border: "none", cursor: "pointer",
                color: "var(--color-dust)", display: "flex", alignItems: "center", gap: 6,
                fontSize: "0.82rem", fontFamily: "var(--font-sans)", padding: "4px 0",
              }}>
            ...
            {editingId && editingAgent && (
              <div style={{ flex: 1, display: "flex", alignItems: "center", gap: 10 }}>
                ...
                <span style={{ fontSize: "0.68rem", color: "var(--color-dust)", fontFamily: "var(--font-mono)" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              {profile?.plan && (() => { const m = planMeta(profile.plan); return (
                <span style={{ padding: "3px 10px", borderRadius: 100, fontSize: "0.68rem", fontWeight: 700, fontFamily: "var(--font-mono)", background: m.bg, color: m.color, border: `1px solid ${m.color}33` }}>
                  {m.label}
                </span>
              ); })()}
        ...
        <main style={{ maxWidth: 1160, margin: "0 auto", padding: "28px 24px", animation: "dashIn 0.3s ease both" }}>
          ...
          {error && (
            <div style={{ marginBottom: 16, padding: "10px 16px", borderRadius: 12, background: "rgba(248,113,113,0.08)", border: "1px solid rgba(248,113,113,0.2)", fontSize: "0.82rem", color: "#f87171" }}>
              {error}
            </div>
          )}
          ...
          {!editingId && (
            <>
              ...
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 22, flexWrap: "wrap", gap: 12 }}>
                <div>
                  <h2 style={{ margin: "0 0 2px", fontFamily: "var(--font-display)", fontWeight: 700, fontSize: "1.15rem", color: "var(--color-starlight)" }}>
                    Your agents
                  </h2>
                  <p style={{ margin: 0, fontSize: "0.78rem", color: "var(--color-dust)" }}>
                    {agents.length} of {agentLimit} agent{agentLimit !== 1 ? "s" : ""} used
                  </p>
                </div>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(320px,1fr))", gap: 16 }}>
                  {agents.map(agent => (
                    <AgentCard
                      ...
                      onViewLeads={() => {
                        const url = `/${agent.subdomain}/leads?key=${agent.id}`;
              ...
              {!canBuild && (
                <div style={{
                  ...
                  <div style={{ flex: 1 }}>
                    <p style={{ margin: "0 0 3px", fontFamily: "var(--font-display)", fontWeight: 600, fontSize: "0.95rem", color: "var(--color-starlight)" }}>
                      Want more agents?
                    </p>
                    <p style={{ margin: 0, fontSize: "0.8rem", color: "var(--color-dust)" }}>
                      Upgrade to Pro for 2 agents, custom URL, and image support in chat.
                    </p>
                  </div>
                  <a href="/pricing" className="btn-genesis" style={{ fontSize: "0.86rem" }}>
                    Upgrade to Pro →
                  </a>
                </div>
              )}
