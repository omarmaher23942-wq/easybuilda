...
export default function ActivateProPage() {
  ...
  if (done) return (
    <div style={{ minHeight:"100vh",background:"#05070f",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:16,textAlign:"center",padding:"2rem" }}>
      ...
      <p style={{ color:"rgba(237,240,247,0.55)",fontSize:"0.92rem" }}>Your agent is now live — no more bills until you get a lead. Redirecting…</p>
    </div>
  );
        ...
        <div style={{ background:"rgba(255,255,255,0.03)",border:"1px solid rgba(237,240,247,0.08)",borderRadius:22,padding:"2.5rem 2rem" }}>
          ...
          <div style={{ display:"flex",flexDirection:"column",gap:12,marginBottom:28 }}>
            {[
              ...
              { n:2, label:"Pay setup fee", done:false, sub: `$${fee.toFixed(2)} one-time — deducted now` },
              { n:3, label:"Agent goes live", done:false, sub: "No more bills until your next lead" },
            ].map(s => (
          ...
          <button onClick={activate} disabled={activating}
            ...
            ) : `Activate Pro — $${fee.toFixed(2)} one-time`}
          </button>
          <p style={{ textAlign:"center",marginTop:10,fontSize:"0.74rem",color:"rgba(237,240,247,0.3)" }}>No subscription. You'll only be charged again per lead.</p>
