...
export default function AgentPage({ params }: { params: Promise<{ username: string }> }) {
  ...
  return (
    <>
      ...
      <div style={{ minHeight: "100vh", display: "flex", flexDirection: "column", background: "var(--color-void)", backgroundImage: `radial-gradient(700px 420px at 60% -5%, rgba(${r},0.1), transparent 65%)` }}>

        {/* Header */}
        <header style={{ display: "flex", alignItems: "center", gap: 12, padding: "14px 20px", borderBottom: "1px solid var(--line)", background: "rgba(5,7,15,0.85)", backdropFilter: "blur(20px)", position: "sticky", top: 0, zIndex: 10 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <p style={{ margin: 0, fontSize: "0.73rem", color: "var(--color-dust)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{agent.tagline || agent.business_name}</p>
          </div>
          <span style={{ fontSize: "0.67rem", padding: "3px 10px", borderRadius: 100, flexShrink: 0, background: "rgba(52,211,153,0.09)", color: "#34d399", border: "1px solid rgba(52,211,153,0.2)" }}>
            online
          </span>
        </header>
