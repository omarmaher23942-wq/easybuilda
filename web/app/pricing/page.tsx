  ...
  return (
    <a href="/" style={{ display: "inline-flex", alignItems: "center", gap: 9, textDecoration: "none" }}>
      <svg viewBox="0 0 1024 1024" width={size} height={size} aria-hidden="true">
        <defs>
          <linearGradient id="ebLogo3" x1="320" y1="232" x2="692" y2="792" gradientUnits="userSpaceOnUse">
            <stop offset="0" stopColor="#a855f7" /><stop offset="0.34" stopColor="#7c3aed" /><stop offset="0.68" stopColor="#2563eb" /><stop offset="1" stopColor="#22d3ee" />
          </linearGradient>
        </defs>
        <path d="M 320 232 L 428 232 L 428 792 L 320 792 Z M 320 232 L 692 232 L 670 319.36 L 320 336 Z M 320 462.08 L 610.16 462.08 L 591.46 545.95 L 320 561.92 Z M 320 688 L 670 704.64 L 692 792 L 320 792 Z" fill="url(#ebLogo3)" />
      </svg>
...
function Check({ color = "#34d399" }: { color?: string }) {
  return <svg width="14" height="14" viewBox="0 0 14 14" fill="none" style={{ flexShrink: 0, marginTop: 2 }}><path d="M2 7l3.5 3.5 6.5-6.5" stroke={color} strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round" /></svg>;
}
  ...
  return (
    <div style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(237,240,247,0.08)", borderRadius: 16, overflow: "hidden" }}>
      <button onClick={() => setOpen(!open)} style={{ width: "100%", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 16, padding: "1.1rem 1.4rem", background: "transparent", border: "none", cursor: "pointer", textAlign: "left" }}>
        ...
        <svg width="18" height="18" viewBox="0 0 18 18" fill="none" style={{ transition: "transform .25s", transform: open ? "rotate(45deg)" : "none", flexShrink: 0, color: "var(--color-dust,#8891a8)" }}>
...
const PLANS = [
  {
    ...
    features: [
      ...
      { text: "Full Pro experience for 3 days", ok: true },
      { text: "Lead capture & dashboard", ok: true },
      { text: "Your own page", ok: true },
  ...
  },
  {
    ...
    features: [
      ...
      { text: "Analytics & insights", ok: true },
...
const FAQS = [
  { q: "Do I need a credit card for the trial?", a: "No. The 3-day trial is completely free. You only need a card if you choose to upgrade after the trial ends." },
  { q: "Can I change my plan later?", a: "Yes, anytime. Upgrade or downgrade from inside your dashboard. Changes take effect immediately on upgrade, or at the next billing cycle on downgrade." },
  { q: "What happens when my trial expires?", a: "Your agent pauses. Your data stays safe. Pick a plan to bring it back — or just let it go. No charge, no drama." },
  ...
  { q: "Can I cancel anytime?", a: "Yes. Cancel from your dashboard — no calls, no forms. Your account stays active until the end of the billing period." },
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--color-void,#05070f)", color: "var(--color-starlight,#edf0f7)", fontFamily: "var(--font-sans,'Inter',sans-serif)", WebkitFontSmoothing: "antialiased", overflowX: "hidden" }}>
      ...
      <div aria-hidden="true" style={{ position: "fixed", inset: 0, zIndex: 0, pointerEvents: "none" }}>
        <div style={{ position: "absolute", top: "-20vh", right: "-10vw", width: "60vw", height: "60vh", borderRadius: "50%", background: "radial-gradient(circle, rgba(124,58,237,0.12), transparent 65%)", filter: "blur(40px)" }} />
        <div style={{ position: "absolute", bottom: "-20vh", left: "-10vw", width: "55vw", height: "55vh", borderRadius: "50%", background: "radial-gradient(circle, rgba(56,189,248,0.08), transparent 65%)", filter: "blur(40px)" }} />
      </div>
      ...
      <main style={{ position: "relative", zIndex: 1 }}>
        ...
        <section style={{ textAlign: "center", padding: "9rem 1.5rem 4rem", maxWidth: 740, margin: "0 auto" }}>
          ...
          <h1 style={{ fontFamily: "var(--font-display,'Sora',sans-serif)", fontWeight: 700, fontSize: "clamp(2.2rem, 5.5vw, 3.8rem)", lineHeight: 1.07, letterSpacing: "-0.025em", marginBottom: "1.2rem" }}>
            Start free.{" "}
            <span style={{ background: "linear-gradient(100deg,#c084fc,#818cf8 35%,#38bdf8 70%,#22d3ee)", WebkitBackgroundClip: "text", backgroundClip: "text", color: "transparent" }}>Scale when ready.</span>
          </h1>
          <p style={{ fontSize: "1.05rem", color: "var(--color-dust,#8891a8)", lineHeight: 1.65, maxWidth: 520, margin: "0 auto" }}>
            3-day Pro trial, no card needed. Pick a plan when you see the leads.
          </p>
        </section>

        {/* Pricing cards */}
        <section style={{ padding: "0 1.25rem 5rem", maxWidth: 900, margin: "0 auto" }}>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(min(100%, 260px), 1fr))", gap: "1.1rem" }}>
            {PLANS.map(plan => (
              <div key={plan.name} style={{ background: plan.highlight ? "rgba(167,139,250,0.04)" : "rgba(255,255,255,0.03)", border: `1px solid ${plan.highlight ? "rgba(167,139,250,0.3)" : plan.popular ? "rgba(56,189,248,0.25)" : line}`, borderRadius: 20, padding: "1.75rem 1.5rem", display: "flex", flexDirection: "column", position: "relative", overflow: "hidden" }}>
                {plan.highlight && <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: 2, background: "linear-gradient(90deg,transparent,rgba(167,139,250,0.7),transparent)" }} />}
                {plan.popular && <span style={{ alignSelf: "flex-start", fontFamily: "var(--font-mono,'JetBrains Mono',monospace)", fontSize: "0.62rem", textTransform: "uppercase", letterSpacing: "0.14em", background: "linear-gradient(135deg,#7c3aed,#2563eb)", color: "#fff", borderRadius: 999, padding: "0.2rem 0.7rem", marginBottom: "0.9rem" }}>Most popular</span>}
                <div style={{ fontFamily: "var(--font-display,'Sora',sans-serif)", fontWeight: 600, fontSize: "1rem", color: plan.color, marginBottom: "0.6rem" }}>{plan.name}</div>
                <div style={{ display: "flex", alignItems: "baseline", gap: 4, marginBottom: "0.35rem" }}>
                  <span style={{ fontFamily: "var(--font-display,'Sora',sans-serif)", fontWeight: 700, fontSize: "2.3rem", letterSpacing: "-0.03em" }}>{plan.price}</span>
                  <span style={{ fontSize: "0.85rem", color: "var(--color-dust,#8891a8)" }}>{plan.period}</span>
                </div>
                <p style={{ fontSize: "0.84rem", color: "var(--color-dust,#8891a8)", marginBottom: "1.4rem" }}>{plan.tagline}</p>
                <ul style={{ listStyle: "none", padding: 0, margin: "0 0 1.6rem", display: "flex", flexDirection: "column", gap: "0.65rem" }}>
                  {plan.features.map(f => (
                    <li key={f.text} style={{ display: "flex", alignItems: "flex-start", gap: 8, fontSize: "0.88rem", color: "var(--color-starlight,#edf0f7)" }}>
                      <Check /> {f.text}
                    </li>
                  ))}
                </ul>
                <a href="/auth/login" style={{ marginTop: "auto", display: "flex", alignItems: "center", justifyContent: "center", gap: 6, padding: "0.75rem 1.2rem", borderRadius: 12, fontFamily: "var(--font-display,'Sora',sans-serif)", fontWeight: 600, fontSize: "0.88rem", textDecoration: "none", transition: "filter .2s, transform .2s", ...(plan.highlight || plan.popular ? { background: "linear-gradient(135deg,#7c3aed,#2563eb 55%,#0ea5e9)", color: "#fff" } : { background: "rgba(255,255,255,0.05)", border: `1px solid ${lineBright}`, color: "var(--color-starlight,#edf0f7)" }) }}
                  onMouseEnter={e => { e.currentTarget.style.filter = "brightness(1.08)"; e.currentTarget.style.transform = "translateY(-1px)"; }}
                  onMouseLeave={e => { e.currentTarget.style.filter = "none"; e.currentTarget.style.transform = "none"; }}>
                  {plan.cta} <Arrow />
                </a>
          ...
          {/* Enterprise callout */}
          <div style={{ marginTop: "1.5rem", background: "rgba(255,255,255,0.02)", border: `1px solid ${line}`, borderRadius: 20, padding: "1.5rem 2rem", display: "flex", flexWrap: "wrap", alignItems: "center", justifyContent: "space-between", gap: 16 }}>
            <div>
              <div style={{ fontFamily: "var(--font-display,'Sora',sans-serif)", fontWeight: 700, fontSize: "1.1rem", color: "#fbbf24", marginBottom: "0.3rem" }}>Enterprise — custom pricing</div>
              <p style={{ fontSize: "0.9rem", color: "var(--color-dust,#8891a8)", lineHeight: 1.5 }}>Unlimited agents · Full white-label · Your domain & branding · Dedicated account manager.</p>
            </div>
        ...
        <section style={{ textAlign: "center", padding: "0 1.5rem 5rem" }}>
          <div style={{ display: "flex", flexWrap: "wrap", alignItems: "center", justifyContent: "center", gap: "2rem", fontSize: "0.88rem", color: "var(--color-dust,#8891a8)" }}>
            {["No credit card for trial", "Cancel anytime", "Your data is always yours", "Support on paid plans"].map(t => (
              <span key={t} style={{ display: "flex", alignItems: "center", gap: 7 }}><Check size={14} /> {t}</span>
            ))}
          </div>
        </section>
        ...
        <section style={{ padding: "0 1.5rem 8rem", maxWidth: 680, margin: "0 auto", textAlign: "center" }}>
          ...
          <a href="/auth/login" style={{ display: "inline-flex", alignItems: "center", gap: 8, background: "linear-gradient(135deg,#7c3aed,#2563eb 55%,#0ea5e9)", color: "#fff", fontFamily: "var(--font-display,'Sora',sans-serif)", fontWeight: 600, fontSize: "1rem", padding: "0.95rem 2.2rem", borderRadius: 12, textDecoration: "none" }}
